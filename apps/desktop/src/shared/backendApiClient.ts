...
export class BackendApiClient {
  ...
  private async fetch<T>(
    ...
    if (payload !== undefined && normalizedHeaders['content-length'] === undefined) {
    ...
    try {
      const responseText = await new Promise<string>((resolve, reject) => {
        ...
        req.setTimeout(this.requestTimeoutMs, () => {
          req.destroy(new Error(`Request timeout after ${this.requestTimeoutMs}ms: ${endpoint}`));
        });
        ...
        if (options.signal) {
          if (options.signal.aborted) {
            ...
            abortListener = () => req.destroy(new Error(`Request aborted: ${endpoint}`));
  ...
  async researchDismissBrief(briefId: string) {
    return this.fetch(`/api/ai/research/briefs/${briefId}`, { method: 'DELETE' });
  }

  // Congress Activity endpoints
  async congressAnalyzeTrade(tradeId: string, tradeData: Record<string, unknown>, model?: string) {
  ...
  async congressAddToWatchlist(ticker: string, reason: string, priority?: number) {
  ...
  async congressDismissFromWatchlist(watchlistId: number) {
    return this.fetch(`/api/ai/congress/watchlist/${watchlistId}`, { method: 'DELETE' });
  }
  ...
  async supplyChainGetCachedMap(cacheKey: string) {
    return this.fetch(`/api/ai/supplychain/cache/${cacheKey}`, { method: 'GET' });
  }

  async supplyChainGetInsights(ticker: string) {
    return this.fetch(`/api/ai/supplychain/insights/${ticker}`, { method: 'GET' });
  }
  ...
  async stewardDismissFinding(findingId: string) {
    return this.fetch(`/api/ai/steward/findings/${findingId}`, { method: 'DELETE' });
  }
  ...
  async stewardApplyTask(taskId: string) {
    return this.fetch(`/api/ai/steward/tasks/${taskId}/apply`, { method: 'POST' });
  }

  // User settings and watchlists endpoints
  async userGetSettings() {
    return this.fetch<{ settings: Record<string, unknown> }>('/api/user/settings', {
      method: 'GET',
    });
  }
  ...
  async userListWatchlists() {
    return this.fetch<{ items: Array<Record<string, unknown>> }>('/api/user/watchlists', {
      method: 'GET',
    });
  }
  ...
  async userUpdateWatchlist(id: number, fields: { symbol?: string; note?: string }) {
  ...
  async publicFlowGetValuations(tickers: string[]) {
    return this.fetch<PublicFlowValuationsResponse>(`/api/publicflow/valuations`, {
      method: 'POST',
  ...
  async congressGetDisclosureLagStats() {
    return this.fetch<CongressDisclosureLagResponse>(`/api/congress/disclosure-lag`, {
      method: 'GET',
    });
  }
}
