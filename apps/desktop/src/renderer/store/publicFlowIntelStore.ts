...
export const usePublicFlowIntelStore = create<PublicFlowIntelState>((set, get) => {
  ...
  return {
    ...
    async loadInitial() {
      ...
      try {
        ...
        const initialTheme = get().selectedThemeId ?? pickFirstTheme(get().selectedWindow);
        if (initialTheme) {
          await get().selectTheme(initialTheme);
        }

        const tickers = (recent ?? []).map((e) => e.ticker).filter(Boolean) as string[];
    ...
    async refresh() {
      ...
      try {
        ...
        set({ lastUpdatedTs: (res as any)?.ts ?? Date.now(), loading: false });
      } catch (err) {
        set({ loading: false, error: formatError(err) });
      }
    },
    ...
    async selectTheme(themeId) {
      ...
      if (!api?.getCandidates) {
        set({ candidatesLoading: false, error: "Candidates IPC endpoint missing" });
        return;
      }

      try {
        const candidates = (await api.getCandidates(themeId)) ?? [];
        set((state) => ({
          candidatesByTheme: { ...state.candidatesByTheme, [themeId]: candidates },
          candidatesLoading: false,
        }));
