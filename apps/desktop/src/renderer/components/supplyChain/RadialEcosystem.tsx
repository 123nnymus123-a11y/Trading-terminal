...
export default function RadialEcosystem({ graph, selectedNodeId, focalNodeId, simulation, onSelectNode }: Props) {
  const { grouped, impactMetrics } = useMemo(() => {
    ...
    graph.edges.forEach((edge) => {
      ...
      const weight = typeof edge.weight === "number" ? edge.weight : edge.criticality ?? 1;
      if (toMetrics) toMetrics.totalImpact += weight;
    });

    // Normalize criticality scores (0-1)
    const maxImpact = Math.max(...Object.values(metrics).map((m) => m.totalImpact), 1);
    ...
    return {
      grouped: {
        direct: graph.nodes.filter((node) => node.tier === "direct" && node.id !== focalNodeId),
        indirect: graph.nodes.filter((node) => node.tier === "indirect" && node.id !== focalNodeId),
        systemic: graph.nodes.filter((node) => node.tier === "systemic" && node.id !== focalNodeId),
      },
  ...
  return (
    <svg viewBox="0 0 800 800" style={{ width: "100%", height: "100%" }}>
      ...
      {(["direct", "indirect", "systemic"] as const).map((tier) => (
        <circle key={tier} cx={400} cy={400} r={RADII[tier]} fill="none" stroke="rgba(148,163,184,0.2)" strokeDasharray="6 10" />
      ))}
      ...
      <Node
        ...
        incomingConnections={impactMetrics[center.id]?.incomingConnections ?? 0}
...
function TierNodes({ nodes, radius, selectedNodeId, failedNodes, onSelect, impactMetrics, impactScores }: TierProps) {
...
function Node({ id, label, x, y, radius, failed, isSelected, onSelect, criticalityScore, incomingConnections, impactScore }: NodeProps) {
  const fill = failed
    ? "rgba(239,68,68,0.5)"
    : impactScore > 0
    ...
    : "rgba(15,23,42,0.75)";
  const stroke = failed ? COLORS.failed : impactScore > 0 ? "#fb923c" : isSelected ? COLORS.selected : COLORS.default;
  ...
  return (
    <g onClick={() => onSelect(id)} style={{ cursor: "pointer" }}>
      ...
      {/* Main node circle */}
      <circle cx={x} cy={y} r={radius} fill={fill} stroke={stroke} strokeWidth={3} />
      
      {/* Company label */}
      <text x={x} y={y - 2} textAnchor="middle" fontSize={10} fill="#e2e8f0" fontWeight="600">{label}</text>
      
      {/* Tooltip info */}
      <title>{`Impact Score: ${(criticalityScore * 100).toFixed(0)}% | Dependencies: ${incomingConnections}`}</title>
