...
export class AiStewardRepo {
  ...
  async listFindings(
    ...
    const params = module ? [userId, tenant, module, limit] : [userId, tenant, limit];
  ...
  async dismissFinding(userId: string, findingId: string, tenantId?: string): Promise<boolean> {
    const tenant = this.resolveTenant(tenantId);
    const result = await this.pool.query(
  ...
  async createTask(
    ...
    const result = await this.pool.query(
      ...
      [id, tenant, userId, taskType, title, description, JSON.stringify(actionData), findingId],
    );
  ...
  async listPendingTasks(userId: string, limit = 20, tenantId?: string): Promise<AiStewardTask[]> {
    const tenant = this.resolveTenant(tenantId);
    const result = await this.pool.query(
  ...
  async getConfig(userId: string, tenantId?: string): Promise<AiStewardConfig | null> {
    const tenant = this.resolveTenant(tenantId);
    const result = await this.pool.query(
  ...
  async setConfig(userId: string, config: Partial<AiStewardConfig>, tenantId?: string): Promise<void> {
    const tenant = this.resolveTenant(tenantId);
    await this.pool.query(
      ...
      [
        ...
        config.notificationPreferences ? JSON.stringify(config.notificationPreferences) : null,
      ],
    );
  }
}
