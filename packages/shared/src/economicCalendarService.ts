...
interface AdapterConfig {
  name: string;
  enabled: boolean;
  fetcher: (config: FetcherConfig) => Promise<import('./economicCalendar').FetcherResult>;
}
...
export async function fetchEconomicEvents(): Promise<EconomicEvent[]> {
  ...
  for (const adapter of adapters) {
    ...
    try {
      ...
      logger.info(`Fetched ${result.events.length} events from ${adapter.name}`);
    } catch (error) {
...
function deduplicateEvents(events: EconomicEvent[]): EconomicEvent[] {
  ...
  for (const event of events) {
    ...
    if (!seen.has(key)) {
      ...
      const existingDataPoints = [existing.forecastValue, existing.previousValue, existing.actualValue].filter(
        (v) => v !== null && v !== undefined,
      ).length;
      const newDataPoints = [event.forecastValue, event.previousValue, event.actualValue].filter(
        (v) => v !== null && v !== undefined,
      ).length;
...
function getAdapterConfig(adapterName: string, env: MainEnv): FetcherConfig {
  const configs: Record<string, FetcherConfig> = {
    FRED: { enabled: !!env.FRED_API_KEY, ...(env.FRED_API_KEY ? { apiKey: env.FRED_API_KEY } : {}) },
    BLS: { enabled: !!env.BLS_API_KEY, ...(env.BLS_API_KEY ? { apiKey: env.BLS_API_KEY } : {}) },
    BEA: { enabled: !!env.BEA_API_KEY, ...(env.BEA_API_KEY ? { apiKey: env.BEA_API_KEY } : {}) },
    Census: { enabled: !!env.CENSUS_API_KEY, ...(env.CENSUS_API_KEY ? { apiKey: env.CENSUS_API_KEY } : {}) },
    TradingEconomics: {
      enabled: !!env.TRADING_ECONOMICS_KEY,
      ...(env.TRADING_ECONOMICS_KEY ? { apiKey: env.TRADING_ECONOMICS_KEY } : {}),
      ...(env.TRADING_ECONOMICS_SECRET ? { secret: env.TRADING_ECONOMICS_SECRET } : {}),
    },
    Finnhub: {
      enabled: !!env.FINNHUB_API_KEY,
    ...
    AlphaVantage: { enabled: !!env.ALPHA_VANTAGE_API_KEY, ...(env.ALPHA_VANTAGE_API_KEY ? { apiKey: env.ALPHA_VANTAGE_API_KEY } : {}) },
  };
 ...
 */
export function queryEconomicEvents(filters?: CalendarFilters): EconomicEvent[] {
  let events = Array.from(eventCache.values());

  if (!filters) {
    return events.sort((a, b) => b.releaseDateTime.getTime() - a.releaseDateTime.getTime());
  }
  ...
  if (filters.categories && filters.categories.length > 0) {
    events = events.filter((e) => filters.categories!.includes(e.eventCategory));
  }
 ...
 */
export async function enrichEventWithAI(event: EconomicEvent): Promise<EconomicEvent> {
  // TODO: Integrate with LLM (Claude, GPT, etc.)
  // For now, generate simple text summary

  if (typeof event.actualValue === 'number' && typeof event.forecastValue === 'number') {
  ...
  if (typeof event.actualValue === 'number' && typeof event.previousValue === 'number') {
...
function applyEventPostProcessing(event: EconomicEvent): EconomicEvent {
  ...
  const statusBoost = event.status === 'released' ? 0.1 : event.status === 'revised' ? 0.05 : 0;
  const importanceBoost = (event.importance / 3) * 0.2;
  const sourceBoost = adapterConfidence.reduce((sum, entry) => sum + entry.score, 0) / Math.max(adapterConfidence.length, 1);
...
function buildAdapterConfidence(event: EconomicEvent): AdapterConfidence[] {
  return event.sources.map((source) => {
    const base = SOURCE_CONFIDENCE[source.name] ?? SOURCE_CONFIDENCE.Other;
    const latencyFactor = source.latencyMs ? clamp(1 - Math.min(source.latencyMs / 120000, 1), 0.2, 1) : 0.85;
    const hint = typeof source.confidenceHint === 'number' ? source.confidenceHint : 1;
...
function describeConfidence(score: number): EconomicEvent['confidenceLabel'] {
